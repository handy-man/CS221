...
public Player getPlayer(String name) {
	...
	try {
		...
		String getPlayerString = "SELECT * FROM `player` WHERE `email` = '" + name + "'";
			results = s.executeQuery(getPlayerString);
...
public boolean createPlayer(String name2, String password2) throws SQLException {
	...
	playerResults = s.executeQuery(playerQuery); //grab entire player table, then if no record exists add them.
