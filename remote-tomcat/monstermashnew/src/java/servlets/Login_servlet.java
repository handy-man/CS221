        ...
        if (ptc != null) {
            if (!email.equals("") && !password.equals("")) {// right or wrong? ->QA             
                if (ptc.login(email, password) == 1) {
                    request.getRequestDispatcher("/monster_farm.jsp").forward(request, response);
                    session.setAttribute("name", email);
                    session.setAttribute("ID", ptc.getPlayer(email).getId());
                    System.out.println(session.getAttribute("ID"));
                    session.setAttribute("money", ptc.getPlayer(email).money);
                } else {
